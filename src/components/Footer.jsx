...
const Game = () => {
    return (
        <div className='sm:py-16 py-12 bg-game-layer bg-cover -mt-1' id='team'>
                ...
                <div className="flex items-center gap-6 lg:flex-row flex-col justify-between">
                    ...
                    <div className="max-w-[530px] relative w-full mt-6 lg:mt-0">
                        <div className="mx-auto w-[114px] relative z-[1] sm:w-[129px] bg-white rounded h-[93px] grid place-items-center">
                            <div className="sm:w-[116px] w-[102px] h-20 rounded bg-vibrantBlue flex flex-col justify-center items-center">
                                <p className='machina_bold font-extrabold text-white text-xl sm:text-2xl text-center'>YOU</p>
                                <p className='font-normal  text-sm text-white text-center'>(Flywheel)</p>
                            </div>
                        </div>
                                ...
                                <div key={idx} className={`mt-9 flex gap-3 sm:gap-6 items-start w-full ${idx === 2 || idx === 3 ? "justify-center gap-3 sm:gap-9" : "justify-between"}`}>
                                    {items.items.map((data, index) => (
                                        <div key={index} className="sm:max-w-[160px] max-w-[103px] h-full shadow-custom-xl w-full px-2 pb-2 min-h-[130px] bg-white pt-[18px] rounded-md flex flex-col items-center">
                                            ...
                                            <p className='mt-2 machina_bold capitalize font-extrabold text-black text-sm sm:leading-5 sm:text-base text-center'>{data.title}</p>
                                            <p className='font-normal capitalize leading-4 text-sm text-black max-w-[123px] text-center mt-1'>{data.text}</p>
                                        </div>
                                    ))}
                                </div>
                            ))}
